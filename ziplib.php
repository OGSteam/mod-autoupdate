<?php
/** $Id: ziplib.php 2739 2007-10-20 13:50:47Z gorn $ **/
/**
* ziplib.php Défini les fonctions zip
...
class ZipLib {
	...
	function get_file() {
		...
		return $data . $ctrldir . $this -> eof_ctrl_dir . pack('v2V2', sizeof($this -> ctrl_dir), sizeof($this -> ctrl_dir), strlen($ctrldir), strlen($data)) . "\x00\x00";
	}
	...
	function ReadCentralDir($zip,$zip_name) {
		...
		while ($pos < $size) {
			$byte = @fread($zip, 1); $bytes=(($bytes << 8) & 0xFFFFFFFF) | Ord($byte);
			if ($bytes == 0x504b0506){
				$pos++;
				break;
			}
	...
	function ExtractFile($header,$to,$zip) {
		...
		if (!isset($header["external"]) or (!($header['external']==0x41FF0010)&&!($header['external']==16))) {
